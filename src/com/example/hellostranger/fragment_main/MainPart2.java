...
import com.example.hellostranger.CreateGroupActivity;
...
import com.example.hellostranger.fragment_main.CopyOfMainPart2.MyHandler;
...
public class MainPart2 extends Fragment implements OnShakeListener {
		...
		for (StrangerBean bean : list) {
			if (!MainActivity.mp1.isFriendExists(bean.strangerId)) {
				...
				OverlayOptions oo = new MarkerOptions().position(ll)
						.icon(userX).zIndex(9).draggable(true);
	...
	public void onShake() {
		if (isshaked) {
			...
			if (!isHidden()) {
				...
				// ((MyApplication)
				// getActivity().getApplication()).getMyLocation();
				setMyLoc(((MyApplication) getActivity().getApplication())
						.getMyLocation());

				if (ClientManager.isShareLoc) {
					/** 发送我的位置，请求得到陌生人列表 */
